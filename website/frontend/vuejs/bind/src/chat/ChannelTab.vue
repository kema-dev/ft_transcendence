<template>
	<router-link v-if="!channel?.psw" :to="{name: 'ChannelConv', params: {conv_name: channel!.name }}" >
  ...
  <div v-else class="channelPsw center column stack">
    ...
    <div v-if="showPsw" class="pswInputCont center raw" id="pswBox">
      ...
      <input v-model="psw" @keydown.enter="checkPsw()" type="text" ref="input" class="pswInput">
    </div>
  </div>
  <WarningMsg v-if="banWarn" msg="You have been banned from this channel"
...
function checkPsw() {
  ...
  setTimeout(() => {
    if (psw.value == props.channel?.psw) {
      router.push({name: 'PrivConv', params: {conv_name: props.channel.name }});
    } else {
...
onMounted(() => {
  chanViewPos = document.getElementById("channel_view")!.getBoundingClientRect()!;
  console.log("height = ", chanViewPos.height, "width = ", chanViewPos.width, "top = ", chanViewPos.top, "right = ", chanViewPos.right, "bottom = ", chanViewPos.bottom, "left = ", chanViewPos.left)
})
...
.pswInputCont {
  z-index: 0;
  height: 2rem;
  animation: slidePsw .3s ease-in-out;
}
@keyframes slidePsw {
...
@keyframes shake {
  ...
  0%     {background-color:rgb(255, 178, 178);}
