...
<div class="center column score">
	<div class="space-between row">
		<h2 class="player" style="text-align: left;" v-if="res == 1">{{player}}</h2>
		<h3 class="player" style="text-align: left;" v-else>{{player}}</h3>
		<h2>{{points1}} | {{points2}}</h2>
		<h2 class="player" style="text-align: right;" v-if="res == 2">{{adversary}}</h2>
...
let res = 0;
if (props.points1 > props.points2)
	res = 1
else if (props.points2 > props.points1)
	res = 2
